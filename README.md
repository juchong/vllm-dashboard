...
- **Model Management** — Download models from HuggingFace Hub with revision selection
  - Background downloads with real-time progress (size, elapsed time)
  - Cancel downloads in progress with automatic cleanup
  - Hide controls during download to prevent conflicts
...
- **YAML Config Editor** — Edit vLLM configuration files with syntax validation
  - Displays config file path for easy reference
  - Default templates for new model configurations
...
| `/api/models/download` | POST | Start background model download (returns task_id) |
| `/api/models/download/status/{task_id}` | GET | Get download progress and status |
| `/api/models/download/active` | GET | List all active downloads |
| `/api/models/download/cancel/{task_id}` | POST | Cancel a download in progress |
| `/api/models/validate/{name}` | GET | Validate model exists on HF |
| `/api/models/revisions/{name}` | GET | Get available model revisions |
| `/api/models/{path}` | DELETE | Delete a downloaded model |
| `/api/models/rename` | POST | Rename a model directory |

### Configuration Files

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/config/model/{model_name}` | GET | Get config and config file path for a model |
| `/api/config/save` | POST | Save configuration for a model |
| `/api/config/associate` | POST | Associate a model with a config file |
| `/api/config/pairs` | GET | List all model-config associations |
...
The `/api/models/download/status/{task_id}` and `/api/models/download/active` endpoints return:
...
```json
{
  ...
  "revision": "main",
  "status": "downloading",
  "progress": "Downloading files from HuggingFace...",
  "error": null,
  "started_at": "2025-01-20T10:30:00+00:00",
  "completed_at": null,
  "downloaded_size": 15728640000,
  "downloaded_size_human": "14.7 GB",
  "elapsed_seconds": 342
}
```
...
│   │   ├── download_manager.py  # Background model downloads
